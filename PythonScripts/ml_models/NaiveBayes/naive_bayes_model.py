...
def get_model_paths(n_graph: int) -> Tuple[str, str]:
    ...
    results_file = os.path.join(RESULTS_DIR, f'naive_bayes_model_results_{n_graph}graph_{timestamp}.txt')
...
class KeystrokeNaiveBayes:
    ...
    def train_and_validate(self):
        ...
        for login in unique_users:
            ...
            # Create and train Naive Bayes model for this user
            model = GaussianNB()
...
def main():
    ...
    for n in [1, 2, 3]:
        print(f"\nTraining Naive Bayes model for {n}-graph...")
        nb = KeystrokeNaiveBayes(n_graph=n)
