...
class KeystrokeLSTMModel:
    ...
    def _open_results_file(self):
        ...
        results_path = os.path.join(self.results_dir, f'lstm_model_results_{self.n_graph}graph_{timestamp}.txt')
    ...
    def train_and_validate(self, tune_hyperparameters=False):
        ...
        self._write_to_results(f"Starting LSTM model training and validation for {self.n_graph}-graph...\n")
...
def main():
    ...
    for n_graph in [1, 2, 3]:
        print(f"\nTraining and validating LSTM models for {n_graph}-graph...")
        lstm = KeystrokeLSTMModel(n_graph=n_graph)
        lstm.train_and_validate(tune_hyperparameters=True)
