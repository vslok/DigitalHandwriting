...
class KeystrokeLSTMModel:
    ...
    def cross_validate(self, X, y, user_id):
        """Perform k-fold cross-validation for a user"""
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
            ...
            model = self.tune_hyperparameters(X_train, y_train, user_id)

            # Evaluate on validation set
            model.eval()
            with torch.no_grad():
                val_preds = model(torch.FloatTensor(X_val).to(device))
            ...
            if fold == 0 or fold_score > max(fold_scores[:-1]):
