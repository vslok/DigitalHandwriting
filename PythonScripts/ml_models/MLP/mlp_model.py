...
def get_model_paths(n_graph: int) -> Tuple[str, str]:
    ...
    results_file = os.path.join(RESULTS_DIR, f'mlp_model_results_{n_graph}graph_{timestamp}.txt')
...
class KeystrokeMLP:
    def __init__(self, n_graph: int = 1, hidden_layer_sizes=(100,), activation='relu', solver='adam', alpha=0.0001, max_iter=200):
...
def main():
    ...
    for n in [1, 2, 3]:
        print(f"\nTraining MLP model for {n}-graph...")
        mlp = KeystrokeMLP(n_graph=n)
        mlp.train_and_validate(tune_hyperparameters=True)
