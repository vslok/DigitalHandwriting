...
class MLPPredictor(BasePredictor):
    def _load_model_and_scaler(self):
        if not os.path.exists(self.model_path) or not os.path.exists(self.scaler_path):
            raise FileNotFoundError(f"MLP Model ({self.model_path}) or scaler ({self.scaler_path}) not found for {self.login}, N_{self.n_value}")
    ...
    def predict_authentication(self, combined_features: np.ndarray) -> bool:
        if self.model is None or self.scaler is None:
            raise RuntimeError("Model or scaler not loaded for MLPPredictor.")
